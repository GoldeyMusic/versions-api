...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics, declaredGenre = null, genreUnknown = false, uploadType = 'master') {
  ...
  // ─── MITIGATION TEMPORAIRE (2026-05-21) ───────────────────────────
  // Le combo uploadType='master' + declaredGenre rempli fait crasher le
  // pipeline depuis le déploiement C.2 du 20 mai. Symptôme côté front =
  // `status:error` au stade `measures_done` après ~105s, sans entrée
  // dans analysis_cost_logs. Cause exacte non identifiée (les try/catch
  // de _analyze.js devraient pourtant absorber tout throw lisible
  // côté code) — il faut les logs Railway au moment du crash pour
  // aller plus loin.
  //
  // En attendant, on neutralise declaredGenre sur ce combo précis et
  // on bascule sur le path "inférence Gemini" qui fonctionne (validé
  // par David le 21 mai à 09:15 UTC). Le genre déclaré est ignoré
  // pour cette analyse — l'inferred_genre sera affiché à la place.
  //
  // À retirer une fois la cause identifiée et le vrai fix déployé.
  if (
    uploadType === 'master'
    && typeof declaredGenre === 'string'
    && declaredGenre.trim().length > 0
  ) {
    console.warn(
      `[claude] mitigation master+declared_genre : ignore "${declaredGenre.trim()}" et bascule en inference (cause crash non resolue, cf CLAUDE.md)`
    );
    declaredGenre = null;
    genreUnknown = true;
  }
...
CONTEXTE DEMO/MAQUETTE — TRIGGER OBLIGATOIRE :

Si l ecoute mentionne explicitement le statut de finition incomplet du
titre — mots-cles strictement deterministes a chercher dans l ECOUTE :
"pour une maquette", "etat de demo", "etat de maquette", "version de
travail", "voix temoin", "voix-temoin", "version voix temoin", "premiere
passe", "ebauche", "version brouillon", "encore en cours", "pas encore
finalisee", "non finalisee", "encore a affiner globalement" — alors :

  (a) La BANDE PAR DEFAUT pour les items LOW DESCEND a 65-78 (au lieu de
      78-84). L echelle 85+ devient INTERDITE sauf validation explicite
      d un niveau d ingenieur du son confirme dans l ecoute ("travail
      d ingenieur du son", "niveau commercial", "production de reference"),
      ce qui est tres rare sur une maquette.

  (b) Justification : un mix au niveau maquette/voix-temoin n est PAS un
      mix professionnel pret au mastering, meme si l ecoute le decrit
      positivement ("bien avance", "soigne", "presque pret"). Ces
      qualificatifs sont des encouragements relativement au statut
      maquette, pas une evaluation absolue du craft d ingenieur du son.

  (c) Exemple concret : ecoute disant "excellent pour une maquette" + items
      decrits comme "voix intelligibles, bien positionnees au centre, avec
      une presence naturelle" -> items 70-78, PAS 80+. La langue "excellent"
      se refere au contexte maquette, pas au niveau pro. Les 3 validations
      ("intelligibles", "bien positionnees", "presence naturelle") sont
      LUES COMME DES DESCRIPTIONS DE BASE COMPETENTE A NIVEAU MAQUETTE,
      pas comme des validations de craft d ingenieur du son confirme.

  (d) Le globalScore d une maquette/demo tombe naturellement dans 55-72.
      Si ta moyenne ponderee t emmene au-dessus de 75 alors que le contexte
      maquette est detecte, force le globalScore a 72 maximum pour rester
      coherent avec le statut de finition incomplete declare par l ecoute.

  (e) Le summary doit reconnaitre le statut maquette explicitement
      ("Maquette deja solidement avancee, avec X et Y a reprendre avant
      de basculer en version mixee" / "Pour une maquette, la base est
      cohrente mais il reste a finaliser X, Y, Z avant le mix final").
      PAS de "presque pret pour un mastering final" sur une maquette —
      le mastering est encore deux etapes plus loin.
`;
